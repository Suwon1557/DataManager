...
| 이름 | 학번 | 역할 | 핵심 구현 사항 |
| :--- | :--- | :--- | :--- |
| **황규희** | 25017095 | 팀장 / UI & 아키텍처 | WinForms 기본 프레임워크 구성, 탭 기반 화면 구조, 반응형 레이아웃, 주요 컨트롤 이벤트 연결, 애플리케이션 상태 관리 |
| **박주원** | 25017039 | 데이터 파이프라인 | Donkeycar 카탈로그 로드, 이미지 경로 매핑, 무결성 검사, 삭제/복구 흐름, 저장본 생성 및 manifest 재작성 |
| **홍시언** | 25017093 | 기능 고도화 & AI | GDI+ 실제값/예측값 오버레이, WSL2 비동기 프로세스 연동, TensorFlow 학습/추론 실행, 밝기 기반 예측 검증 |
...
4. `ParseCatalogData(_allData, imageBasePath, catalogFiles)`가 각 catalog JSON 라인을 읽어 `DrivingData` 객체를 만듭니다.
...
* `chtTestSpeedValue`는 실제 속도값과 예측 속도값을 함께 표시합니다.
...
2. `필터링` 버튼으로 조향값 또는 속도값이 0인 후보 프레임을 제거합니다.
